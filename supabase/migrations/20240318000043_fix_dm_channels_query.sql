...
-- Create improved function with fixed window functions
...
BEGIN
    ...
    WITH latest_messages AS (
        SELECT 
            dm.channel_id,
            first_value(dm.created_at) OVER (PARTITION BY dm.channel_id ORDER BY dm.created_at DESC) as last_message_at,
            first_value(dm.content) OVER (PARTITION BY dm.channel_id ORDER BY dm.created_at DESC) as last_message_content
    ...
    message_counts AS (
        ...
        AND dm.created_at > dmm.last_read_at
        GROUP BY dm.channel_id
    )
    SELECT 
        dmm.channel_id,
        other_member.user_id as other_user_id,
        other_member.profile_id as other_profile_id,
    ...
    JOIN direct_message_members other_member ON 
        other_member.channel_id = dmm.channel_id AND 
        other_member.user_id != user_uuid
    JOIN profiles p ON p.id = other_member.profile_id
